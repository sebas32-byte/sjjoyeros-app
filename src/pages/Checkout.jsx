...
export default function Checkout() {
  ...
  return (
    <main className="section-shell mx-auto max-w-3xl px-4 py-16">
      <h1 className="mb-4 font-display text-5xl leading-[0.95]">Checkout</h1>
      {submitted ? (
        <div className="rounded-[1rem] border border-white/10 bg-white/5 p-6">
          ...
          <p className="mt-4 text-sm font-semibold text-gold">Estado inicial: Pendiente de confirmacion.</p>
          <a href="/#catalogo" className="luxury-btn-secondary mt-5 inline-flex items-center justify-center px-6 py-3 text-sm font-semibold">Continuar comprando</a>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <input required value={name} onChange={(event) => setName(event.target.value)} className="luxury-input w-full p-3" />
            <label className="block text-sm text-white/70">Teléfono o WhatsApp</label>
            <input required value={phone} onChange={(event) => setPhone(event.target.value)} className="luxury-input w-full p-3" />
            <label className="block text-sm text-white/70">Correo electrónico (opcional)</label>
            <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="luxury-input w-full p-3" />
            <label className="block text-sm text-white/70">Notas</label>
            <textarea value={notes} onChange={(event) => setNotes(event.target.value)} className="min-h-[100px] w-full rounded-md border border-white/10 bg-white/5 p-3 text-white" />
          </div>

          {error ? <p className="rounded-md border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-300">{error}</p> : null}

          <div className="flex gap-3">
            <button type="submit" disabled={loading} className="luxury-btn-primary px-6 py-3 font-semibold disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? 'Procesando…' : 'Enviar pedido'}
            </button>
            <a href="/" className="luxury-btn-secondary inline-flex items-center justify-center px-6 py-3 text-sm font-semibold">Cancelar</a>
          </div>
        </form>
      )}
    </main>
